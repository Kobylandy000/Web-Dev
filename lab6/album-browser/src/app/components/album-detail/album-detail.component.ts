...
@Component({
    ...
    <div class="detail-container">
      ...
      <div *ngIf="!loading && album" class="detail-card">
        ...
        <div class="edit-section">
          ...
          <input type="text" [(ngModel)]="editTitle" class="edit-input" placeholder="Enter new title..." />
          <button class="btn-save" (click)="saveTitle()">{{ saved ? '✓ Saved!' : 'Save Changes' }}</button>
        </div>
        <div class="action-buttons">
          <a [routerLink]="['/albums', album.id, 'photos']" class="btn-photos">🖼️ View Photos</a>
        </div>
      </div>
    </div>
  `,
  styles: [`
    ...
    .album-meta { display: flex; gap: 16px; margin-bottom: 16px; }
